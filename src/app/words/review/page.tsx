  ...
  useEffect(() => {
    ...
    const fetchWords = async (): Promise<void> => {
      try {
        ...
        /** 🔹 単語ごとの統計計算 */
        const historyStats = new Map<string, { total: number; correct: number; lastAnswered: string }>();
        history?.forEach((h) => {
        ...
        const reviewWords = userWordsWithStats.filter((w) => {
          ...
          const isFullyMemorized = w.correct_count >= 6 && w.successRate! >= 0.9;
          if (isFullyMemorized) return false;

          if (w.correct_count >= 6 && w.successRate! < 0.9) {
  ...
  const handleAnswer = async (isOk: boolean): Promise<void> => {
    try {
      ...
      await supabase.from("user_word_history").insert({
        user_word_id: current.id,
        user_id: userId,
        is_correct: isOk,
        answered_at: now,
      });

      const newCount = isOk ? current.correct_count + 1 : 0;

      await supabase.from("user_words").update({ correct_count: newCount }).eq("id", current.id);
  ...
  return (
    <div className="p-4 flex justify-center">
      <div className="w-full max-w-xl">
        ...
        <p className="text-sm text-gray-500 mb-4 text-center">{currentIndex + 1} / {words.length}</p>

        <div className="bg-white shadow-lg rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-indigo-600">{m.word}</h2>
            <button onClick={() => speakText(m.word)} className="text-indigo-500 hover:text-indigo-700 text-xl">🔊</button>
          </div>

          <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
            <span className="text-lg">{m.example_sentence}</span>
            <button onClick={() => speakText(m.example_sentence)} className="text-blue-500 hover:text-blue-700 text-xl">🔊</button>
          </div>

          {!showAnswer ? (
            <button onClick={() => setShowAnswer(true)} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 rounded-xl transition">答えを見る</button>
          ) : (
            <div className="space-y-3">
              <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                ...
                <p className="text-sm text-gray-600">
                  正解数: {current.correct_count} 回 / 誤答数: {current.wrong} 回
                </p>
                  ...
                  <span
                    className={
                      current.successRate! >= 0.8
                        ? "text-green-600"   // 80%以上 → 緑
                        : current.successRate! >= 0.5
                        ? "text-yellow-600"  // 50～79% → 黄
                        : "text-red-600"     // 50%未満 → 赤
                    }
                  >
              ...
              <div className="flex gap-4">
                <button onClick={() => handleAnswer(true)} className="flex-1 bg-green-500 hover:bg-green-600 text-white font-semibold py-2 rounded-xl transition">OK</button>
                <button onClick={() => handleAnswer(false)} className="flex-1 bg-red-500 hover:bg-red-600 text-white font-semibold py-2 rounded-xl transition">NG</button>
              </div>
            </div>
          )}
        </div>
